...
#
# SYMBOL LEGEND:
# Ctx: N% Context usage percentage (of 200k context window)
# Git information:
...
: "${context_length:=0}"
ctx_percentage=$(awk -v val="$context_length" 'BEGIN {printf "%.0f", (val / 200000) * 100}')
...
output_string="${BOLD}${BLUE}${model_name}${RESET} ${GRAY}│${RESET} ${CYAN}Ctx: ${ctx_percentage}%${RESET} ${GRAY}│${RESET} ${PURPLE}${dir_display}${RESET}"
